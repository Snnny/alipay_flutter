...
//    controller.positions.elementAt(1).jumpTo(_computedHeight(item, index));
  ...
  double _computedHeight(String group, int index) {
    // 取出缓存数据
    if(cacheData.containsKey(index)) {
      return cacheData[index];
    }
    double rpx = MediaQuery.of(context).size.width / 750;
    double rowHeight = 130 * rpx;
    double _height = 0;
    if(index == 0) {
    } else {
      List<String> _groupList = groupList.take(index).toList();
      _height += titleHeight * _groupList.length;
      for(var item in _groupList) {
        _height += result[item].length * rowHeight;
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0,
        actions: <Widget>[
          Icon(Icons.add, color: Colours.app_main_light, size: 30,),
        ],
      ),
      body: Stack(
        children: <Widget>[
  ...
  void afterFirstLayout(BuildContext context) {
    RenderBox box = stickyKey.currentContext.findRenderObject();
    Offset offset = box.localToGlobal(Offset.zero);
    setState(() {
      zeroHeight = offset.dy;
    });
  }
}
